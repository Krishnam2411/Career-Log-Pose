...
class RootAgent:
    """
    A class wrapper for your root agent.
    It initializes the ADK agent and provides an `invoke` method for the API.
    """
    def __init__(self):
        self.agent = Agent(
    ...
    async def invoke(self, inputs: dict, configurable: dict | None = None) -> dict:
        """
        This method is the entry point for the API to interact with your agent.
        It passes the call directly to the underlying ADK agent's invoke method.
        """
        return await self.agent.invoke(inputs, configurable)
